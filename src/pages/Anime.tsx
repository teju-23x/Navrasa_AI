        ...
        <div className="max-w-4xl mx-auto relative z-10">
          ...
          >
            ...
            <h1 className="text-5xl md:text-8xl font-display tracking-tighter text-accent-red leading-none drop-shadow-lg relative">
              Anime <span className="text-navy">Discovery</span>
            </h1>
          </motion.div>
          <p className="mt-4 text-navy text-lg md:text-2xl font-black uppercase tracking-[0.4em] opacity-80">
            ★ <span className="italic font-serif normal-case tracking-normal">Eastern Masterpieces</span> ★
          </p>
          <p className="mt-2 text-accent-gold text-xl md:text-3xl font-display">Animated Tales from Across the Sea</p>
        </div>
      </section>
      ...
      <div className="px-4 md:px-10 pb-20 mt-12">
        ...
        <div className="relative w-full max-w-3xl group mx-auto">
          ...
          <input
            ...
            placeholder="Search anime... e.g. Dark fantasy, Studio Ghibli style"
          ...
          >
            {loading ? <div className="w-6 h-6 border-4 border-white/30 border-t-white rounded-full animate-spin" /> : 'Discover'}
          </button>
        </div>

        {/* Quick Tags */}
        <div className="mt-12 flex flex-wrap gap-3 justify-center">
          {['Cyberpunk', 'Shounen', 'Studio Ghibli', 'Dark Fantasy', 'Slice of Life'].map((tag) => (
            <button
              key={tag}
              onClick={() => { setAnimeSearchQuery(tag); setTimeout(generateAnimeRecs, 100); }}
          ...
          <div className="mt-20">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-6 md:gap-10">
              <AnimatePresence mode="popLayout">
                {animeRecommendations.map((item, idx) => (
        ...
        {!hasResults && !loading && !error && (
          <div className="mt-20 text-center py-20 bg-white border-[4px] border-navy border-dashed max-w-2xl mx-auto">
            <div className="text-navy/20 mb-6 flex justify-center"><Zap size={64} /></div>
            <h3 className="text-2xl font-display text-navy mb-2">READY TO BROADCAST</h3>
            <p className="text-navy/40 font-black uppercase tracking-widest text-[10px]">Search for anime to begin your discovery</p>
          </div>
        )}
        ...
        {error && (
          <div className="mt-20 text-center py-20 bg-red-50 border-[4px] border-accent-red border-dashed max-w-2xl mx-auto">
            ...
            <button onClick={() => generateAnimeRecs()} className="vintage-button px-10 h-12">Retry Signal</button>
          </div>
        )}
      </div>
    </div>
